		...
		<z-paging ref="paging" v-model="list" @query="loadData" :refresher-enabled="false" watch-touch-direction-change @scrollTopChange="scroll"
			...
			<view slot="top">
				<view class="top">
					...
					<view class="search-box" :style="{height: !scrollTop && !hideSearchDom ? '270px' : 0 }">
						<u-form label-width="66px">
							<view class="form-box">
								...
								<u-form-item label="订单类型" borderBottom>
									<r-select v-model="type" placeholder="请选择订单类型" :list="typeList" />
								</u-form-item>
								<u-form-item v-if="type === '1'" label="订单状态" borderBottom>
									<r-select v-model="params.operatorStatus" placeholder="请选择订单状态" :list="operatorStatusList" />
								</u-form-item>
								<u-form-item v-if="type === '2'" label="订单状态" borderBottom>
									<r-select v-model="params.statusCode" placeholder="请选择订单状态" :list="platStatusList" />
								</u-form-item>
								<u-form-item v-if="type === '3'" label="方式" borderBottom>
									<r-select v-model="params.visitMode" placeholder="请选择拜访方式" :list="visitModeList" />
								</u-form-item>
								<u-form-item v-if="type === '3'" label="关键字" borderBottom>
									<u-input v-model="params.keywords" border="none" placeholder="请输入关键字" />
								</u-form-item>
								<u-form-item v-else label="关键字" borderBottom>
									<u-input v-model="params.keywords" border="none" placeholder="姓名,地址,电话,工单号,服务等" />
								</u-form-item>
							</view>
						</u-form>
			...
			<view v-if="list.length" class="list">
				...
				<template v-if="type === '2'">
					<r-order-plat v-for="l in list" :key="l.id" :item="l" isBtn />
				</template>
				<template v-if="type === '3'">
					<r-visiting v-for="l in list" :key="l.id" :item="l" />
				</template>
			</view>
		</z-paging>

		<u-datetime-picker :show="showTime" v-model="time" mode="date" closeOnClickOverlay @confirm="timeConfirm" @cancel="timeCancel"
			@close="timeCancel"></u-datetime-picker>
	</view>
</template>
	...
	export default {
		data() {
			return {
				list: [],
				total: 0,
				params: { page: 1, keywords: '', operatorStatus: '', statusCode: '', serveBeginTime: '', serveEndTime: '' },
				time: Date.now(),
				showTime: false,
				timeKey: '',
				hasSearch: false,
		...
		computed: {
			...
			platStatusList() {
				// 服务商才有5的待审核tab
				const list = this.aDict('order_status').slice(4)
				return [{ value: '', label: '全部' }, ...list]
			},
			visitModeList() {
				const list = this.aDict('visit_mode')
				return [{ value: '', label: '全部' }, ...list]
			}
		},
		onLoad() {
		...
		methods: {
			async loadData(current) {
				...
				if (this.type === '3') {
					...
					if (this.type === '1') {
						const res = await uni.$u.http.post(staffInfoPageUrl, this.params)
						data = res.data
					} else {
						const res = await uni.$u.http.post(orderPageUrl, this.params)
						data = res.data
					}
					// 构建是否同一天数据逻辑处理
					data.list.forEach(d => {
			...
			handleClear() {
				...
				this.params = { page: 1, keywords: '', operatorStatus: '', statusCode: '', serveBeginTime, serveEndTime }
				this.type = '1'
				// this.reload()
			},
			handleOpenTime(key) {
