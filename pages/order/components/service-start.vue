<template>
	<view class="add-page">
		<u-navbar title="开始服务确认" placeholder bgColor="#FAF9F7">
			<view slot="left" class="u-flex">
				<text class="u-tips-color" @click="handleBack">取消</text>
			</view>
			<view slot="right" class="u-flex">
				<text class="u-primary" @click="handleConfirm">确认</text>
			</view>
		</u-navbar>

		<view class="add-card">
			<view class="u-m-b-8">
				房屋地址门牌号 <text class="u-error">(“提交审核”时必填，最多3张)</text>
			</view>
			<r-upload v-model="form.startAddressPicture" :imgList="startAddressPictureAttach" :maxCount="3" />
		</view>

		<view class="add-card">
			<view class="u-m-b-8">
				与服务对象合照 <text class="u-info">(最多上传3张)</text>
			</view>
			<r-upload v-model="form.startObjectPicture" :imgList="startObjectPictureAttach" :maxCount="3" />
		</view>

		<view class="add-card">
			<view class="u-m-b-8">
				备注内容
			</view>
			<r-textarea v-model="form.startServeRemark" count :maxlength="300" placeholder="请输入内容" class="min-textarea"></r-textarea>
		</view>

		<r-dialog v-model="startShow" title="确认开始服务？" cancelText="取消" confirmText="确定" @cancel="startShow = false" @confirm="dialogConfirm">
			<view slot="content" class="u-flex">
				<text class="u-text-center">提交后不可修改状态，请确保已到达服务时间和地址。是否确认开始服务?</text>
			</view>
		</r-dialog>

		<r-dialog v-model="show" title="监测到未保存信息" cancelText="否" confirmText="是" @cancel="show = false" @confirm="toBack">
			<view slot="content">
				<text class="u-text-center">当前页面已做出修改，是否不保存就退出</text>
			</view>
		</r-dialog>

		<r-location ref="rLocation" />
	</view>
</template>

<script>
	import { getStaffInfoDetailUrl, startServiceUrl } from '@/api'

	export default {
		data() {
			return {
				unsave: false,
				startShow: false,
				show: false,
				loading: false,
				isFirst: true,
				form: { startAddressPicture: '', startObjectPicture: '', startServeRemark: '' },
				startAddressPictureAttach: [],
				startObjectPictureAttach: []
			}
		},
		watch: {
			form: {
				handler: function() {
					this.unsave = true
				},
				deep: true
			}
		},
		onLoad({ id }) {
			this.form.id = id
			this.loadData()
		},
		methods: {
			async loadData() {
				const { data } = await uni.$u.http.post(`${getStaffInfoDetailUrl}?id=${this.form.id}`)
				this.isFirst = !data.startOrderTime
				if (!this.isFirst) {
					this.startAddressPictureAttach = data.startAddressPictureAttach || []
					this.startObjectPictureAttach = data.startObjectPictureAttach || []
					this.form.startAddressPicture = data.startAddressPicture
					this.form.startObjectPicture = data.startObjectPicture
					this.form.startServeRemark = data.startServeRemark
				}
			},
			handleBack() {
				if (this.unsave) {
					this.show = true
					return
				}
				uni.navigateBack()
			},
			toBack() {
				uni.navigateBack()
			},
			async handleConfirm() {
				if (this.isFirst) {
					const { longitude, latitude, address } = await this.$refs.rLocation.getAddress()
					this.form.startLongitude = longitude
					this.form.startLatitude = latitude
					this.form.startOrderAddress = address
					this.startShow = true
					return
				}
				this.dialogConfirm()
			},
			async dialogConfirm() {
				await uni.$u.http.post(startServiceUrl, this.form)
				uni.navigateBack()
			}
		}
	}
</script>

<style scoped lang="scss">
	.add-page {
		box-sizing: border-box;
		background-color: #FAF9F7;
		position: relative;
		min-height: 100vh;
		padding: 12px 16px 24px;
	}

	.add-card {
		background: #ffffff;
		border-radius: 8px;
		padding: 12px;
		margin-bottom: 12px;
	}

	.address-text {
		font-size: 14px;
		color: #333231;
		margin-bottom: 14px;
	}

	.address-tip {
		background: #fff9f0;
		border-radius: 4px;
		font-size: 12px;
		color: #ff9500;
		padding: 2px 5px;
		margin-right: 5px;
	}

	/deep/ .u-textarea--no-radius {
		border: none;
	}
</style>