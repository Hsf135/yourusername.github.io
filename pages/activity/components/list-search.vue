<template>
	<u-popup :show="showSearch" mode="right" :custom-style="{ backgroundColor: '#faf9f7' }">
		<u-navbar title="活动记录搜索" placeholder bgColor="#faf9f7">
			<u-icon slot="left" :name="aIcon('zuojiantou')" width="24" height="24" @click="handleReturn"></u-icon>
		</u-navbar>
		<view class="search-box">
			<u-search placeholder="可输入活动标题、内容、社区" bgColor="#fff" v-model="params.keyword" :showAction="false" shape="square"></u-search>
			<u-form label-width="66px">
				<view class="form-box">
					...
					<u-form-item label="活动性质" borderBottom>
						<r-select v-model="params.assort" placeholder="请选择活动性质" :list="aDict('assort')" />
					</u-form-item>
					<u-form-item label="活动类型" borderBottom>
						<r-select v-model="params.type" placeholder="请选择活动类型" :list="aDict('comactivty_type')" />
					</u-form-item>
					<u-form-item label="活动分类" borderBottom>
						<r-select v-model="params.activitySort" placeholder="请选择活动分类" :list="aDict('activity_sort')" />
					</u-form-item>
				</view>
			</u-form>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			handleConfirm () {
				if (this.params.beginTime && this.params.endTime) {
					if (new Date(this.params.beginTime) > new Date(this.params.endTime)) return uni.showToast({ title: '开始时间不得大于结束时间', icon: 'none', duration: 2000 })
				}
				let str = ''
				...
				uni.navigateTo({
					url: `/pages/activity/search-result?${str.slice(0, str.length - 1)}`
				})
			},
			handleClear () {
				this.params.keyword = ''
				this.params.activitySort = ''
				this.params.assort = ''
				this.params.type = ''
