<template>
	<view class="full-page">
		<z-paging ref="paging" v-model="list" @query="loadData" :auto="false" bg-color="#fff" empty-view-img="/static/imgs/common/empty.png"
			:emptyViewText="type === 'search' ? '试试关键字搜索老人~' :'暂无老人信息~'">
			<view slot="top" class="box">
				...
				<view v-if="type==='search'" class="search u-flex">
					...
					<u-input v-model="search" focus confirmType="search" placeholder="搜索关键字" border="none" @confirm="reload">
						<u-icon slot="suffix" :name="aIcon('search')" width="24" height="24" @click="reload"></u-icon>
					</u-input>
				</view>
				<view class="total">
					<view v-if="params.isSearch">- 已查询过的老人 - </view>
					<view v-else>
						<text>- 共有 </text>
			...
			<view v-if="list.length" class="list">
				<r-man v-for="l in list" :key="l.id" :item="l" isCall :isSearch="type === 'search'" :from="from" />
			</view>
		</z-paging>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				type: 'search',
				list: [],
				params: { pageNum: 1, pageSize: 15, appType: 1 },
				search: '',
				total: 0,
				searchType: 'name',
				searchTypeName: '姓名',
