	...
	<view>
		...
		<view class="item" @click="handleDetail">
			...
			<view class="u-flex u-m-b-8">
				...
				<text class="u-main-color">{{radioDict(item.belongProject,'belong_project')}}</text>
			</view>
			<view class="u-flex u-m-b-8">
			...
			<view v-if="isBtn" class="u-border-top u-p-t-14 u-flex u-flex-around">
				...
				<view v-if="['3', '4', '8'].includes(item.status)" @click.stop="handlePhone(item)">
...
<script>
	export default {
		...
		methods: {
			...
			handleDetail() {
				uni.navigateTo({ url: `/pages/order/components/status?id=${this.item.id}` })
			},
			getStatus(item) {
